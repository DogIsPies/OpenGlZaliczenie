...
public class Prostopadloscian {
	...
	private int t;
	public static void Draw(GL2 gl,float a,float b,float h,int n,int m,int p,int t){
...
//					if((i+j)%2==0) gl.glColor3f(0.0f,0.0f,0.0f);
					...
					gl.glTexCoord2f(1.0f,0.0f);gl.glVertex3f(a/2,-h/2+h*(j+1)/(p-1),-b/2+b*i/(m-1));
					gl.glTexCoord2f(1.0f,1.0f);gl.glVertex3f(a/2,-h/2+h*(j+1)/(p-1),-b/2+b*(i+1)/(m-1));
					gl.glTexCoord2f(0.0f,1.0f);gl.glVertex3f(a/2,-h/2+h*j/(p-1),-b/2+b*(i+1)/(m-1));
					gl.glEnd();
					gl.glPopAttrib();
				}
			gl.glRotatef(180,0,0,1);
